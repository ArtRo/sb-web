...
public class ScanController implements ApplicationRunner {
    ...
    @Value("${scan.controller}")
    Boolean isScanController;
    ...
    public void scanController() {
        if (isScanController) {
            ...
            try {
                scanController.doScan(basePath, searchPath);
            } catch (Exception e) {
                e.printStackTrace();
                throw new ApplicationRunTimeExeption();
            }
        }
    }

    @Transactional(rollbackFor = ApplicationRunTimeExeption.class, propagation = Propagation.REQUIRED)
    public void doScan(String pkePath, String path) {
